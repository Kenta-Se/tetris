...
# Option 取得
###############################
def get_option(game_time, mode, drop_interval, random_seed, obstacle_height, obstacle_probability, resultlogjson,
               train_yaml, predict_weight, user_name, ShapeListMax, BlockNumMax, art_config_filepath):
...
class Game_Manager(QMainWindow):
    ...
    def __init__(self):
        ...
        if args.mode in (
        "keyboard", "gamepad", "sample", "art", "train", "predict", "train_sample", "predict_sample", "train_sample2",
        "predict_sample2"):
            self.mode = args.mode
        ...
        if event.timerId() == self.timer.timerId():
            ...
            if BLOCK_CONTROLLER and not self.nextMove:
                ...
                if self.mode == "sample":
                    ...
                    from machine_learning.block_controller_train_sample import \
                        BLOCK_CONTROLLER_TRAIN_SAMPLE as BLOCK_CONTROLLER_TRAIN
                    self.nextMove = BLOCK_CONTROLLER_TRAIN.GetNextMove(nextMove, GameStatus, yaml_file=self.train_yaml,
                                                                       weight=self.predict_weight)

                elif self.mode == "train_sample2" or self.mode == "predict_sample2":
                    # sample train/predict
                    # import block_controller_train_sample, it's necessary to install pytorch to use.
                    from machine_learning.block_controller_train_sample2 import \
                        BLOCK_CONTROLLER_TRAIN_SAMPLE2 as BLOCK_CONTROLLER_TRAIN
                    self.nextMove = BLOCK_CONTROLLER_TRAIN.GetNextMove(nextMove, GameStatus,
                                                                       yaml_file="config/train_sample2.yaml",
                                                                       weight=self.predict_weight)
...
class Board(QFrame):
    ...
    def updateData(self):
        ...
        if self.game_time == -1:
            ...
            print("  1 line: " + str(line_Score["line1"]) + " * " + str(line_score_stat[0]) + " = " + str(
                line_Score["line1"] * line_score_stat[0]))
            print("  2 line: " + str(line_Score["line2"]) + " * " + str(line_score_stat[1]) + " = " + str(
                line_Score["line2"] * line_score_stat[1]))
            print("  3 line: " + str(line_Score["line3"]) + " * " + str(line_score_stat[2]) + " = " + str(
                line_Score["line3"] * line_score_stat[2]))
            print("  4 line: " + str(line_Score["line4"]) + " * " + str(line_score_stat[3]) + " = " + str(
                line_Score["line4"] * line_score_stat[3]))
            print("  dropdownscore: " + str(dropdownscore))
            print("  gameover: : " + str(line_Score["gameover"]) + " * " + str(gameover_count) + " = " + str(
                line_Score["gameover"] * gameover_count))
